  ...
  return (
    <div
      style={{
        ...
        background: "linear-gradient(145deg, #070811, #0c0a17 55%, #090b12)",
      }}
    >
      <div
        style={{
      ...
      >
        {[0, 1, 2].map((dot) => (
          <span
            key={dot}
            style={{ width: 7, height: 7, borderRadius: 99, background: "rgba(255,255,255,.24)" }}
          />
        ))}
        <span style={{ marginLeft: "auto", fontSize: 7, letterSpacing: ".18em", opacity: 0.45 }}>
          ANMOL / COMPONENTS
        </span>
      </div>
      <div style={{ position: "relative", zIndex: 2, padding: "56px 38px" }}>
        <div style={{ fontSize: 8, letterSpacing: ".2em", fontWeight: 800, opacity: 0.5 }}>
          INTERACTIVE COMPONENT
        </div>
        <div style={{ marginTop: 9, fontSize: 43, lineHeight: 0.95, fontWeight: 800, letterSpacing: "-.055em" }}>
          3D Laptop
        </div>
        <div style={{ marginTop: 13, maxWidth: 260, fontSize: 10, lineHeight: 1.55, opacity: 0.56 }}>
          Built completely from React Three Fiber primitives. No external 3D model required.
        </div>
      </div>
    </div>
  );
...
function LaptopModel({
  ...
  return (
    <group rotation={[0, -0.28, 0]} position={[0, -0.12, 0]}>
      ...
      <RoundedBox args={[4.65, 0.04, 2.22]} radius={0.06} smoothness={4} position={[0, -0.405, 0.1]} receiveShadow>
        <meshStandardMaterial color="#22252a" roughness={0.48} metalness={0.55} />
      </RoundedBox>

      <Keyboard />

      <RoundedBox args={[1.9, 0.025, 0.78]} radius={0.06} smoothness={4} position={[0, -0.392, 1.2]} receiveShadow>
      ...
      <group position={[0, -0.33, -1.61]} rotation={[-0.1, 0, 0]}>
        ...
        <Html transform position={[0, 1.52, 0.128]} distanceFactor={1.45} style={{ pointerEvents: "none" }}>
          <div style={{ width: 480, height: 288, overflow: "hidden", borderRadius: 15, background: "#05060a" }}>
  ...
  return (
    <div className={className} style={{ width: "100%", height: "100%", minHeight: 280 }}>
      <Canvas
        dpr={[1, 1.75]}
        shadows
        camera={{ position: [7, 4.3, 7.3], fov: 34, near: 0.1, far: 100 }}
      ...
      >
        ...
        <Suspense fallback={null}>
          <Float speed={1.4} rotationIntensity={0.08} floatIntensity={0.26} floatingRange={[-0.06, 0.06]}>
